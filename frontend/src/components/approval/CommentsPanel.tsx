    ...
    <div className="rounded-xl border border-white/10 bg-zinc-900/70">
      ...
      <div className="flex items-center justify-between border-b border-white/10 px-5 py-3.5">
        ...
        {selected.length > 0 && (
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg bg-fuchsia-600 px-3 py-1.5 text-xs font-medium transition hover:bg-fuchsia-500 disabled:opacity-40"
          >
            {runningAI ? (
            ...
            {runningAI ? "Optimizing" : `Apply AI (${selected.length})`}
          </button>
        )}
      </div>
      ...
      <div className="max-h-80 overflow-y-auto">
        {comments.length === 0 ? (
          ...
          comments.map((c) => (
            <div key={c.id} className="border-b border-white/5 px-5 py-3.5">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  {c.selected_text && (
                    <div className="mt-2 rounded-lg border border-yellow-500/20 bg-yellow-500/5 px-3 py-2 text-sm italic text-yellow-200">
                      "{c.selected_text}"
                    </div>
                  )}
                ...
                <div className="flex shrink-0 items-center gap-1">
                  ...
                  <button
                    ...
                    className="rounded-md p-1.5 text-red-400 transition hover:bg-red-500/10 disabled:cursor-not-allowed disabled:opacity-25"
                  >
                    <Trash2 size={15} />
                  </button>
                </div>
      ...
      <div className="space-y-2.5 border-t border-white/10 p-4">
        <input
          ...
          className="w-full rounded-lg border border-white/10 bg-zinc-950 px-3 py-2 text-sm outline-none focus:border-cyan-500 disabled:opacity-50"
        />
        ...
        <div className="flex justify-end">
          <button
            ...
            onClick={submit}
            title="Add comment"
            className="inline-flex items-center gap-2 rounded-lg bg-cyan-600 px-3 py-2 text-sm transition hover:bg-cyan-500 disabled:opacity-40"
          >
            {saving ? (
