...
export default function SowApprovalViewer({
  loading,
  viewerRole,
  sow,
  comments,
  ...
  if (!sow || !document || !version) {
    return (
      <div className="rounded-2xl border border-white/10 bg-zinc-900/70 p-8">
        <p className="text-zinc-500">Select a SOW from the left panel.</p>
      </div>
    );
  }

  const canApprove =
    viewerRole === document.current_stage && document.status !== "Approved";
  ...
  async function handleExport(format: string) {
    setExporting(true);
    try {
      const res = await fetch(`${API_BASE}/sow/export`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          sow: markdown,
          format,
          template_id: exportTemplateId || undefined,
          state: null,
          transcript: null,
        }),
      });
          ...
          <div className="flex shrink-0 items-center justify-between border-t border-white/10 px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative" ref={exportMenuRef}>
                ...
                {exportMenuOpen && (
                  <div className="absolute right-0 z-40 mt-2 w-64 rounded-xl border border-white/10 bg-zinc-900 p-3 shadow-2xl">
              ...
              <button
                ...
                title={
                  canApprove ? "Approve this SOW" : "Not your turn to approve"
                }
